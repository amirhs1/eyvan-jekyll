...
```text
       Refill Rate (R tokens/sec)
         ...
         │   ◌   ◌   ◌   │      when Bucket Max Capacity (B)
         │ ◌   ◌   ◌   ◌ │      is reached.
         └───────┬───────┘
                 │
                 │ Request arrives (Requires 1 Token)
                 ▼
         /───────────────\
        /  Tokens Avail?  \
        \_________________/
              ...
              /     \ Yes (Consume 1 Token)
             ▼       ▼
       [ Reject ]   [ Allow Request ]
        HTTP 429     Forwarded Downstream
...
![Token Bucket System Design Diagram]({{ page.image | relative_url }})
*Figure 1 Visual breakdown of the high-level system architecture, illustrating request flow, token consumption, and refill mechanics (Image generated by Gemini).*
